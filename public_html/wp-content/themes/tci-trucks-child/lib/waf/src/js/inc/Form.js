...
class Form extends EventEmitter {
    constructor(data = {}, args = {}) {
        ...
        Object.entries(data).forEach(el => {
            ...
            else if (d.type === 'duplicator') {
                // d('add duplicator',d,k);
                this.fields.push(new Duplicator(d, k, this));
            }
    ...
    get $messages() {
        // d(this.sel,$(this.sel)[0]);
        if ($(this.sel + '> .messages ').length == 0 ) $(this.sel).append('<div class="messages"></div>');
        return $(this.sel + '> .messages:last');
    }
    get $bd() {
        return $(this.sel + ' .bd');
    }
    ...
    render($el) {
        ...
        getDataAttr(context);
        // d('context:',context.data,context.dataAttr,context);
    ...
    renderValidation() {
        ...
        // d("INVLIAD:", labels);
        const messageStr = this.args.data.invalid;
    ...
    // Form Data
    getFormData(extend,el) {
        const formData = new FormData();
        
        // d('getting for mdata?');
        if( !el ) el = this;
        el.allFields().forEach(field => {
            // d('field:', field.context);
            if (field.context.saveData === false) {
                // d('-- skip', field.name);
                return;
            }
            // d(field.parent);
            if (field.parent) if (field.parent.context.saveData === false) {
                // d('-- skip', field.parent.name);
                return;
            }
            if (field.context.type == 'fieldset' || field.context.type == 'duplicator') return;
            const value = field.value;
            // d('-- FIELD VLAUE:', field.name, value);
            
            
            if (field.context.type == 'file' || field.$input.attr('type') == 'file' ) {
                
                if (field.context.files) {
                    d('files!');
                    for (var file of field.context.files) {
                        // d(file);
                        if (file) {
                            if (file.src)
                                formData.append(field.context.nameAttr, file.src, file.name);
                            formData.append(field.context.nameAttr, JSON.stringify({ id: file.id, name: file.name }));
                            d(field.context.nameAttr, JSON.stringify({ id: file.id, name: file.name }));
                        }
                    }
                } else {
                    d('add files', field.$input[0].files,field.$input[0]);
                    if (field.$input[0].files) for (var file of field.$input[0].files) {
                        d('add file', file);
                        formData.append(field.context.nameAttr, file, file.name);
                    }
                }
                // d('files:',field.name,field.context.files);
            } else {
                const name = pathToName(field.getPath(true));
                // d(name,value);
    ...
    getFormJSON(fields = this.fields, data = {}) {
        ...
        let files = [];
        if (this.method) if (this.method.toLowerCase() != 'get') {
            this.allFields().forEach(field => {
                if (field.context.type == 'file') {
    ...
    submit(extend,extendedArgs={}) {
        ...
        if (this.invalid) {
            if( !this.args.skipValidation && !extendedArgs.skipValidation ) {
        ...
        if( extendedArgs ) {
            // d(extendedArgs);
            if (extendedArgs.url) url = extendedArgs.url;
            if (extendedArgs.method) method = extendedArgs.method;
        }
        if( this.route ) url = this.route;
        else this.route = url;
        ...
        if (format == 'json' || method == 'get') data = { ...data, ...extend }
        else if (format == 'serialize' ) data = this.$form.serialize();
        else data = this.getFormData(extend)
        ...
        // d(JSON.stringify(this.getFormJSON()));
        // d(data);

        // TODO: Sort out the formats on this thing.  this is a fix only.
        if( format == 'serialize' && method == 'get' ) data = this.getFormJSON();
        ...
        promise
            .then(response => {
                ...
                // d("COMPLETE", this.$form[0], response);
                // d('h',response.headers);
                this.removeLoading();
                this.disabled = false;
                this.changed = false;
                // d(this.$form[0]);
                const completeAction = this.autosaving  ? 'autosave-complete' : 'complete';
                if( this.args.returnHeaders && !this.autosaving ) this.$form.trigger( completeAction, response);
                else this.$form.trigger(completeAction, response.data);
            ...
            .catch((res) => {
                ...
                // console.log(res);
                // d('[Promise Error submittiting]:',res);
                // d(res.messages[0]);
                if (this.autosaving) this.$form.trigger('autosave-failed', res)
                else this.$form.trigger('failed',res)
                this.autosaving = false;
                this.$form.trigger('failed', res)
                this.disabled = false;
                return false;
            });
        // } else {
    ...
    message(data, clear = true, field = null) {
        ...
        if (this.args.toastr && toastr[tslug] && ( data.mode == 'toastr' || data.mode == undefined ) ) {
            //d('toast it up',data,status,message);
            toastr[tslug](message);
        }

        if (this.args.inlineMessages && (data.mode == 'inline' || data.mode == undefined) ) {
            if (clear && field) field.$errors.html('');
            else if (clear) this.$messages.html('');
            if( field ) if( field.$errors.length == 0 ) field.$field.append('<div class="errors"></div>');
    ...
    set value(values) {
        ...
        fields.forEach(field => {
            ...
            if (field instanceof Fieldset) return;
            if (data) {
                field.value = data.value;
                field.context.value = data.value;
                // d('set value',field.context,data.value);
                // if (field.context.type == 'radios') d('radios field:',field.context);
            }
        })
    }
    set(fieldName, value) {
