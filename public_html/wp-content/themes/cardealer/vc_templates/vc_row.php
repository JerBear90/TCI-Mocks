...
$css_classes[] = ( empty( $cd_bg_type ) ) ? 'row-background-light' : $cd_bg_type;
...
global $mpc_paths;
$is_mpc_installed = ( $mpc_paths ) ? true : false;
...
if ( ! $is_mpc_installed && filter_var( $cd_enable_overlay, FILTER_VALIDATE_BOOLEAN ) ) {
	if ( isset( $cd_overlay_color ) && ! empty( $cd_overlay_color ) ) {
		$overlay_html .= 'background-color: ' . esc_attr( $cd_overlay_color ) . ';';
	}
	if ( isset( $cd_overlay_opacity ) && ! empty( $cd_overlay_opacity ) ) {
		$overlay_html .= 'opacity: ' . esc_attr( (int) $cd_overlay_opacity / 100 ) . ';';
	}
	if ( ! empty( $overlay_html ) ) {
		$overlay_html = '<div class="vc_row-background-overlay" style="' . $overlay_html . '"></div>';
	}
}
...
echo $output; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotE
