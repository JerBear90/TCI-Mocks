...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function setup_wizard_header() {
			...
			<body class="envato-setup wp-core-ui envato-setup-step-<?php echo esc_attr( $this->step ); ?>" <?php cardealer_body_styles(); ?>>
				<h1 id="wc-logo">
					<?php
					$header_logo = sprintf(
		...
		public function is_plugin_check_active( $slug ) {
			...
			return (
				...
				&& (
					...
					|| cardealer_check_plugin_active( $instance->plugins[ $slug ]['file_path'] )
				)
			);
		}
				...
				<div class="sample-contents-wrapper clearfix">
					<div class="sample-contents">
						...
						foreach ( $sample_datas as $sample_data_k => $sample_data ) {
							...
							$sample_data['previwe_img_path'] = false;

							if (
								( isset( $sample_data['previwe_img'] ) && ! empty( $sample_data['previwe_img'] ) && filter_var( $sample_data['previwe_img'], FILTER_VALIDATE_URL ) )
								&& ( 0 === strpos( $sample_data['previwe_img'], $theme_url ) )
							) {
								$sample_data['previwe_img_path'] = str_replace( $theme_url, untrailingslashit( $theme_dir ), $sample_data['previwe_img'] );
							}

							$sample_data_classes   = array( 'sample-content' );
							$sample_data_classes[] = 'sample-content-' . $sample_data_k;
							$imported              = filter_var( ( isset( $sample_data['is_imported'] ) ? $sample_data['is_imported'] : false ), FILTER_VALIDATE_BOOLEAN );
							...
							<div class="<?php echo esc_attr( $sample_data_classes ); ?>" data-version="<?php echo esc_attr( $sample_data_k ); ?>">
								<div class="sample-content-view">
									<?php
									if ( isset( $sample_data['previwe_img_path'] ) && $sample_data['previwe_img_path'] && file_exists( $sample_data['previwe_img_path'] ) ) {
										?>
										<div class="sample-content-thumb">
											<img src="<?php echo esc_url( $sample_data['previwe_img'] ); ?>" alt="<?php echo esc_attr( $sample_data['name'] ); ?>"/>
										</div>
										<?php
									} else {
										?>
