...
if ( ! function_exists( 'cardealer_vehicle_image_gallery_view360_button' ) ) {
	function cardealer_vehicle_image_gallery_view360_button( $vehicle_id = '' ) {
		if ( ! $vehicle_id ) {
			$vehicle_id = get_the_ID();
		}

		$vehicle_360_images = cardealer_get_vehicle_360_images( $vehicle_id );
		if ( $vehicle_360_images && is_array( $vehicle_360_images ) && ! empty( $vehicle_360_images ) ) {
			?>
			<div class="view-360-btn"><?php cardealer_vehicle_360_view_button_html( $vehicle_360_images ); ?></div>
			<?php
		}
	}
}
...
function cardealer_vehicle_listing_page_mobile_filters() {
	...
	$mobile_listing_sidebar = isset( $car_dealer_options['mobile_listing_sidebar'] ) ? $car_dealer_options['mobile_listing_sidebar'] : 'enable';

	if ( 'enable' === $mobile_listing_sidebar ) {
...
if ( ! function_exists( 'cardealer_get_offcanvas' ) ) {
	...
	function cardealer_get_offcanvas() {
		...
		$mobile_listing_filter      = isset( $car_dealer_options['enable_mobile_listing_filter'] ) ? $car_dealer_options['enable_mobile_listing_filter'] : 'enable';
		$mobile_off_canvas_position = isset( $car_dealer_options['vehicle_listing_mobile_off_canvas_position'] ) ? $car_dealer_options['vehicle_listing_mobile_off_canvas_position'] : 'left';

		if ( wp_is_mobile() ) {
			if ( 'enable' === $mobile_listing_filter ) {
				$listing_sidebar = $mobile_off_canvas_position;
			} else {
				return;
			}
		}
...
function cardealer_vehicle_list_item_seller_contacts() {
	global $car_dealer_options, $post;

	$display_seller_contacts = ( isset( $car_dealer_options['cdfs_display_seller_contacts'] ) ) ? $car_dealer_options['cdfs_display_seller_contacts'] : 'no';
	$display_seller_contacts = filter_var( $display_seller_contacts, FILTER_VALIDATE_BOOLEAN );

	if ( $display_seller_contacts && $post ) {
		$contact_items = ( isset( $car_dealer_options['cdfs_display_seller_contact_items'] ) && ! empty( $car_dealer_options['cdfs_display_seller_contact_items'] ) ) ? $car_dealer_options['cdfs_display_seller_contact_items'] : array();
		if ( ! empty( $contact_items ) ) {
			$contacts_data = array();
			foreach ( $contact_items as $contact_item => $v ) {
				if ( filter_var( $v, FILTER_VALIDATE_BOOLEAN ) ) {
					if ( 'email' === $contact_item ) {
						$cdfs_show_email = filter_var( get_the_author_meta( 'cdfs_show_email', $post->post_author ), FILTER_VALIDATE_BOOLEAN );
						if ( $cdfs_show_email ) {
							$email = cdfs_get_user_email( $post->post_author );
							if ( $email ) {
								$contacts_data[ $contact_item ] = array(
									'label' => esc_html__( 'Email', 'cardealer' ),
									'value' => $email,
								);
							}
						}
					} elseif ( 'phone' === $contact_item ) {
						$cdfs_show_phone = filter_var( get_the_author_meta( 'cdfs_show_phone', $post->post_author ), FILTER_VALIDATE_BOOLEAN );
						if ( $cdfs_show_phone ) {
							$phone       = cdfs_get_user_phone( $post->post_author );
							if ( $phone ) {
								$contacts_data[ $contact_item ] = array(
									/* translators: %s phone number. */
									'label' => esc_html__( 'Call', 'cardealer' ),
									'value' => cdfs_get_user_phone_url( $phone, $post->post_author ),
								);
							}
						}
					} elseif ( 'whatsapp' === $contact_item ) {
						$cdfs_show_whatsapp = filter_var( get_the_author_meta( 'cdfs_show_whatsapp', $post->post_author ), FILTER_VALIDATE_BOOLEAN );
						if ( $cdfs_show_whatsapp ) {
							$whatsapp = cdfs_get_user_whatsapp( $post->post_author );
							if ( $whatsapp ) {
								$contacts_data[ $contact_item ] = array(
									'label' => esc_html__( 'WhatsApp', 'cardealer' ),
									'value' => $whatsapp,
								);
							}
						}
			...
			if ( ! empty( $contacts_data ) ) {
				...
				<div class="seller-contacts">
					<?php
					foreach ( $contacts_data as $contact => $data ) {
						if ( 'whatsapp' === $contact ) {
							?>
							<a href="<?php echo esc_url( "https://wa.me/{$data['value']}" ); ?>" class="seller-contact seller-contact-<?php echo esc_attr( $contact ); ?>" target="_blank" rel="noreferrer noopener">
								<i class="fab fa-whatsapp"></i><?php echo esc_html( $data['label'] ); ?>
							</a>
							<?php
						} elseif ( 'email' === $contact ) {
							?>
							<a href="<?php echo esc_url( "mailto:{$data['value']}" ); ?>" class="seller-contact seller-contact-<?php echo esc_attr( $contact ); ?>">
								<i class="far fa-envelope"></i><?php echo esc_html( $data['label'] ); ?>
							</a>
							<?php
						} elseif ( 'phone' === $contact ) {
							?>
							<a href="<?php echo esc_url( $data['value'] ); ?>" class="seller-contact seller-contact-<?php echo esc_attr( $contact ); ?>">
								<i class="fas fa-phone"></i><?php echo esc_html( $data['label'] ); ?>
							</a>
							<?php
						}
					}
