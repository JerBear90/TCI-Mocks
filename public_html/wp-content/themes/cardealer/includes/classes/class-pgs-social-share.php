...
class PGS_Social_Share {
	...
	public static function get_params( $type = 'data', $post = 0, $encoded = true ) {
		...
		$params = array(
			'title'      => ( $encoded ) ? urlencode( get_the_title( $post ) ) : get_the_title( $post ),
			...
			'summary'    => ( $encoded ) ? urlencode( get_the_excerpt( $post ) ) : get_the_excerpt( $post ),
			'site_title' => ( $encoded ) ? urlencode( get_bloginfo( 'name' ) ) : get_bloginfo( 'name' ),
