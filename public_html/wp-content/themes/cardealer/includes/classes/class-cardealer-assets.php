...
class CarDealer_Assets {
	...
	function vehicle_detail_page_scripts( $script_data, $script_key ) {
		...
		if ( is_singular( 'cars' ) || is_singular( 'cardealer_template' ) ) {
			...
			if ( 'photoswipe' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}

			if ( 'photoswipe-ui-default' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}

			if ( 'jaaulde-cookies' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}

			if ( 'cardealer-compare' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}
		}
	...
	function vehicle_detail_page_styles( $script_data, $script_key ) {
		...
		if ( is_singular( 'cars' ) || is_singular( 'cardealer_template' ) ) {
			...
			if ( 'photoswipe-default-skin' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}

			if ( 'timepicker' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}

			if ( 'cardealer-contact-form' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}
	...
	function blog_styles( $script_data, $script_key ) {
		...
		if ( is_search() || ( ( is_author() || is_category() || is_home() || is_single() || is_tag() || is_date() ) && 'post' === get_post_type() ) ) {
			...
			// Conditionally enqueue Owl Carousel CSS on blog pages (matches blog_scripts owl-carousel JS).
			if ( 'owl-carousel' === $script_key ) {
				$script_data['action'] = 'enqueue';
			}
		}
	...
	function additional_styles( $script_data, $script_key ) {
		...
		if ( isset( $_REQUEST['action'] ) && in_array( $_REQUEST['action'], $actions ) || isset( $_REQUEST['elementor-preview'] ) ) {
			...
			// Conditionally enqueue WooCommerce CSS on WooCommerce pages.
			if ( 'cardealer-woocommerce' === $script_key && class_exists( 'WooCommerce' ) ) {
				if ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() ) {
					$script_data['action'] = 'enqueue';
				}
			}

			// Conditionally enqueue Owl Carousel CSS on WooCommerce product pages.
			if ( 'owl-carousel' === $script_key && class_exists( 'WooCommerce' ) && is_product() ) {
				$script_data['action'] = 'enqueue';
			}

			// Conditionally enqueue Contact Form CSS on singular pages with contact form shortcode.
			if ( 'cardealer-contact-form' === $script_key && is_singular() ) {
				$post_content = get_post_field( 'post_content', get_the_ID() );
				if ( has_shortcode( $post_content, 'cardealer_contact_form' ) ) {
					$script_data['action'] = 'enqueue';
				}
			}
		}
	...
	function inventory_styles( $script_data, $script_key ) {
		...
		if ( is_post_type_archive( 'cars' ) || is_singular( 'cars' ) || is_singular( 'cardealer_template' ) || is_page_template( 'templates/sold-cars.php' ) || cardealer_is_tax_page() ) {
			...
			// Enqueue owl-carousel CSS on inventory pages with carousel style (matches inventory_scripts).
			if ( 'owl-carousel' === $script_key && 'carousel' === $featured_vehicles_list_style ) {
				$script_data['action'] = 'enqueue';
			}
		...
		$core_scripts =  array(
			...
			'cardealer-vehicle-detail' => array(
				...
				'localize' => array(
					'vehicle_detail_js' => array(
						'ajaxurl'            => admin_url( 'admin-ajax.php' ),
						'gallery_image_loop' => ( isset( $car_dealer_options['vehicle-single-gallery-image-loop'] ) && ! empty( $car_dealer_options['vehicle-single-gallery-image-loop'] ) ) ? (bool) $car_dealer_options['vehicle-single-gallery-image-loop'] : false,
					),
				),
			),
			...
			'cardealer_admin_js' => array(
				...
				'localize' => array(
					'cardealer_admin_js' => array(
						'ajaxurl'                   => admin_url( 'admin-ajax.php' ),
						'set_possition'             => esc_html__( 'Set Possition', 'cardealer' ),
			...
			'js-cloudimage-360-view' => array(
				'handle'    => 'js-cloudimage-360-view',
				'src'       => CARDEALER_URL . '/js/library/js-cloudimage-360-view/js-cloudimage-360-view.min.js',
				'ver'       => '3.2.0',
				'deps'      => array(),
				'in_footer' => true,
				'action'    => 'register',
			...
			'cardealer_view360' => array(
				'handle'    => 'cardealer_view360',
				'src'       => CARDEALER_URL . '/js/frontend/view360' . $suffix . '.js',
				'ver'       => '6.0.0',
				'deps'      => array( 'js-cloudimage-360-view' ),
				'in_footer' => true,
				'action'    => 'register',
				'context'   => array(
					'front',
	...
	private function get_styles() {
		...
		$library_styles = array(
			...
			*/
			...
			/*
			 * Font Awesome v4 Shims — Audit Findings
			 *
			 * The theme uses FA6 syntax throughout (fas, far, fab prefixes) with
			 * standard FA6 icon names (e.g. fa-search, fa-user, fa-shopping-cart).
			 * However, third-party plugins and user-generated content may still
			 * reference legacy FA4 icon names that require the v4 compatibility
			 * shim to resolve correctly.
			 *
			 * Decision: Keep font-awesome-shims globally enqueued (front + admin)
			 * for now to avoid breaking plugin or user content that relies on
			 * FA4 class names. Re-evaluate once all active plugins are confirmed
			 * to use FA6 naming exclusively.
			 */
		...
		$core_styles = array_merge(
			...
			array(
				...
				'cardealer-sidebar' => array(
					...
					'src'       => CARDEALER_URL . '/css/frontend/sidebar' . $suffix . '.css',
					'deps'      => array(),
					'ver'       => CARDEALER_VERSION,
					'action'    => 'register',
					'context'   => array(
	...
	private function google_fonts_url() {
		...
		if ( empty( $font_family ) ) {
			...
			if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'cardealer' ) ) {
				$fonts[] = 'Open+Sans:ital,wght@0,300;0,400;0,600;0,700;0,800;1,300;1,400;1,600;1,700;1,800';
			}
			...
			if ( 'off' !== _x( 'on', 'Roboto font: on or off', 'cardealer' ) ) {
				$fonts[] = 'Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,700;1,900';
			}
		}
		...
		if ( $fonts ) {
			...
			foreach ( $fonts as $font ) {
				$family_params[] = 'family=' . $font;
			}
			$fonts_url = 'https://fonts.googleapis.com/css2?' . implode( '&', $family_params ) . '&display=swap';
		}
		return $fonts_url;
	}
}
