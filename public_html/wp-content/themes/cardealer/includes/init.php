	...
	function cardealer_wpsearch( $form ) {
		...
		<form role="search" method="get" class="search-form clearfix" action="<?php echo esc_url( home_url( '/' ) ); ?>" >
			<div class="search cd-search-wrap">
				<label class="screen-reader-text"><?php echo esc_html__( 'Search for:', 'cardealer' ); ?></label>
				<input type="search" value="<?php echo esc_attr( get_search_query() ); ?>" name="s" class="cardealer-default-search cd-search-autocomplete-input" placeholder="<?php echo esc_attr__( 'Search...', 'cardealer' ); ?>" data-seach_type="default" />
				<input type="submit" class="search-submit cd-search-submit" value="<?php echo esc_attr__( 'Go', 'cardealer' ); ?>" />
