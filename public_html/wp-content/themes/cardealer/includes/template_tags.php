					...
					<a href="<?php echo esc_url( get_permalink( $prev_post->ID ) ); ?>">
						<div class="port-photo pull-left">
							<?php if ( has_post_thumbnail( $prev_post->ID ) ) : ?>
								...
								<?php } else { ?>
								<img class="cd-post-nav-image" src="<?php echo esc_url( $prev_post_thumb_url ); ?>" alt="<?php echo esc_attr( get_the_title( $prev_post->ID ) ); ?>">
								<?php } ?>
					...
					<a href="<?php echo esc_url( get_permalink( $next_post->ID ) ); ?>">
						...
						<div class="port-photo">
							<?php if ( has_post_thumbnail( $next_post->ID ) ) : ?>
								...
								<?php } else { ?>
								<img class="cd-post-nav-image" src="<?php echo esc_url( $next_post_thumb_url ); ?>" alt="<?php echo esc_attr( get_the_title( $next_post->ID ) ); ?>">
								<?php } ?>
								<?php
							else :
...
if ( ! function_exists( 'cardealer_team_social_profiles' ) ) {
	/**
	 * Team social profiles.
	 */
	function cardealer_team_social_profiles( $post_id = '' ) {
		if ( empty( $post_id ) ) {
			global $post;
			$post_id = $post->ID;
		}
		...
		$services        = cardealer_team_social_profile_services();
		foreach ( $services as $service_k => $service_data ) {
			$acf_service_url = get_post_meta( $post_id, $service_k, true );
			if ( $acf_service_url ) {
				$social_profiles[ $service_k ] = array_merge( $service_data, array(
					'url' => $acf_service_url,
				) );
			}
		}
...
if ( ! function_exists( 'cardealer_team_social_profiles_html' ) ) {
	/**
	 * Team social profiles.
	 */
	function cardealer_team_social_profiles_html( $post_id = '' ) {
		$social_profiles = array();
		if ( empty( $post_id ) ) {
			global $post;
			$post_id = $post->ID;
		...
		$social_profiles = cardealer_team_social_profiles( $post_id = '' );
		if ( ! empty( $social_profiles ) ) {
			?>
			<div class="team-social">
				<ul class="team-social-items">
					<?php
					$social_profile_index = 1;
					foreach ( $social_profiles as $social_profile_k => $social_profile_data ) {
						$item_classes = array(
							'team-social-item',
							"team-social-item-{$social_profile_k}",
						);
						$link_classes = array(
							"team-social-icon-{$social_profile_index}",
							"icon-{$social_profile_index}",
						);
						?>
						<li class="<?php cardealer_class_generator( $item_classes ); ?>">
							<a class="<?php cardealer_class_generator( $link_classes ); ?>" href="<?php echo esc_url( $social_profile_data['url'] ); ?>">
								<i class="<?php echo esc_attr( $social_profile_data['icon'] ); ?>"></i>
							</a>
						</li>
						<?php
						$social_profile_index++;
					}
					?>
				</ul>
			</div>
