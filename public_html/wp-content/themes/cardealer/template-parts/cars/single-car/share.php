...
$social_share_inline_links     = ( isset( $car_dealer_options['social_share_inline_links'] ) ) ? $car_dealer_options['social_share_inline_links'] : false;
...
global $share_el_count;

$share_el_count = ( ! isset( $share_el_count ) ) ? 1 : $share_el_count + 1;
$post_id        = $post->ID;
...
$modal_id = "cardealer-share-modal-{$post_id}-{$share_el_count}";
$modal_label_id = "cardealer-share-modal-label-{$post_id}-{$share_el_count}";
?>
<div class="<?php cardealer_class_generator( $cardealer_share_classes ); ?>">
	<div class="cardealer-share-action">
		...
		<div class="cardealer-share-action-item cardealer-share-action-more">
			<a class="cardealer-share-action-link" href="javascript:void(0)" data-modal_id="<?php echo esc_attr( $modal_id );?>" title="<?php echo esc_html__( 'Click to share', 'cardealer' ); ?>" data-native_params="<?php echo esc_attr( wp_json_encode( $native_share_params ) ); ?>"><i class="fa-solid fa-share-nodes"></i></a>
		</div>
		<?php
					...
					<ul class="cardealer-share-items single-share-box mk-box-to-trigger" data-title="<?php echo esc_attr( $share_title ); ?>" data-url="<?php echo esc_url( $share_link ); ?>" data-image="<?php echo esc_url( $share_image ); ?>">
						...
						foreach ( $share_medias as $share_media_k => $share_media_v ) {
							...
							?><li class="<?php echo esc_attr( $item_class ); ?>">
								<a href="<?php echo esc_url( $share_media_v['share_link'] ); ?>" class="<?php echo esc_attr( $link_class ); ?>" target="_blank">
									<?php
	...
	<!-- Modal -->
	<div class="cardealer-share-modal modal fade" id="<?php echo esc_attr( $modal_id );?>" tabindex="-1" role="dialog" aria-labelledby="<?php echo esc_attr( $modal_label_id ); ?>">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title" id="<?php echo esc_attr( $modal_label_id ); ?>"><?php esc_html_e( 'Share This', 'cardealer' ); ?></h4>
				</div>
