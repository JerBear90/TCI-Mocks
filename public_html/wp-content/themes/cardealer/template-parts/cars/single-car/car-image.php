...
$vehicle_single_gallery_image_count = ( isset( $car_dealer_options['vehicle-single-gallery-image-count'] ) && ! empty( $car_dealer_options['vehicle-single-gallery-image-count'] ) ) ? $car_dealer_options['vehicle-single-gallery-image-count'] : 'no';
...
<div class="slider-slick">
	...
	if ( function_exists( 'get_field' ) ) {
		...
		<div class="my-gallery cars-image-gallery">
			...
			<?php
			if ( 'yes' === $vehicle_single_gallery_image_count && '3' !== $layout && $images ) {
				?>
				<div class="slider-count"></div>
				<?php
			}
			do_action( 'after_image_gallery_slider', $post_id );
			?>
		</div>
		<?php
			...
			<div class="slider slider-nav">
				...
				if ( $images ) {
					...
					foreach ( $images as $image ) {
						$imag_alt = ( '' !== $image['alt'] ) ? $image['alt'] : get_the_title();
						?>
						<img class="vehicle-detail-gallery-nav-img img-responsive" src="<?php echo esc_url( $image['sizes']['car_thumbnail'] ); ?>" alt="<?php echo esc_attr( $imag_alt ); ?>">
						<?php
					}
				}
				?>
