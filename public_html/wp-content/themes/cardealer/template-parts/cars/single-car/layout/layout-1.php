	...
	<div class="col-lg-12 col-md-12">
		...
		<div class="car-detail-post-option">
			<ul>
				...
				$video_link      = get_post_meta( $vehicle_id, 'video_link', true );
				if ( ! empty( $video_link ) ) {
					?>
					<li>
				...
				if ( $vehicle_360_images && is_array( $vehicle_360_images ) && ! empty( $vehicle_360_images ) ) {
					?>
					<li><?php cardealer_vehicle_360_view_button_html( $vehicle_360_images ); ?></li>
					<?php
				}
				if ( ! isset( $car_dealer_options['is-compare-on-vehicle-detail'] ) || 'yes' === $car_dealer_options['is-compare-on-vehicle-detail'] ) {
					?>
					<li><a href="javascript:void(0)" title="<?php echo esc_attr( get_the_title() ); ?>" data-id="<?php echo esc_attr( $vehicle_id ); ?>" class="pgs_compare_popup compare_pgs"><i class="fas fa-exchange-alt"></i> <?php esc_html_e( 'Add to compare', 'cardealer' ); ?></a></li>
					<?php
				}
				...
				if ( class_exists( 'CDFS_Wishlist' ) && 'yes' === $wishlist_status && is_user_logged_in() ) {
					...
					if ( $cdfs_wishlist->is_car_in_wishlist( $vehicle_id ) ) {
					...
					?>
					<li>
						<a href="javascript:void(0)" title="<?php echo esc_attr( get_the_title() ); ?>" data-id="<?php echo esc_attr( $vehicle_id ); ?>" class="<?php echo esc_attr( $wishlist_class ); ?>">
...
<div class="row">
    ...
    if ( 'left' === $sidebar_position ) {
        ...
        <div class="col-lg-4 col-md-4 col-sm-5">
            ...
            <?php
			$custom_sidebar = get_post_meta( $vehicle_id, 'custom_sidebar', true );
			if ( $custom_sidebar ) {
